...
export default function ChatPanel({
  ...
  setStep, isLoading, setIsLoading
}) {
  ...
  useEffect(() => {
    if (chatHistory.length === 1 && architecture?.ambiguities?.length > 0) {
      askNextQuestion([]);
    } else if (chatHistory.length === 1 && (!architecture?.ambiguities || architecture.ambiguities.length === 0)) {
      setStep('diagram');
    }
  }, []);
    ...
    try {
      ...
      if (result.done) {
        ...
        setChatHistory(prev => [...prev, {
          role: 'assistant',
          content: '✅ Got everything I need! Generating your architecture diagram now...',
          options: [],
        }]);
  ...
  const handleSend = async (message) => {
    ...
    if (questionCount >= 3) {
      ...
      setChatHistory(prev => [...prev, {
        role: 'assistant',
        content: '✅ All clarifications complete! Generating your architecture diagram now...',
        options: [],
      }]);
      setTimeout(() => setStep('diagram'), 1200);
      return;
    ...
    <div className="card" style={{ maxWidth: 800, margin: '0 auto', display: 'flex', flexDirection: 'column', height: 600 }}>
      <div style={{ marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid #e2e8f0' }}>
        ...
        <p style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>
          Answer a few questions to generate an accurate diagram
        </p>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16, paddingBottom: 8 }}>
        {chatHistory.map((msg, i) => (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
            ...
            {msg.context && (
              <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 4, maxWidth: '80%' }}>
                {msg.context}
              </p>
            )}

            {msg.options && msg.options.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8, maxWidth: '80%' }}>
        ...
        {isLoading && (
          <div style={{ display: 'flex', alignItems: 'flex-start' }}>
            <div style={{
              ...
              padding: '10px 14px', fontSize: 14, color: '#64748b'
            }}>
              <span className="spinner" style={{ borderColor: 'rgba(0,0,0,0.1)', borderTopColor: '#3b82f6' }} />
              Thinking...
            </div>
          </div>
        )}
